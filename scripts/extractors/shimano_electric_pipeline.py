...
from scripts.extractors import shimano_electric_detail, shimano_electric_index, shimano_electric_spec
...
def run_pipeline(index_json_path: str | Path, crawl_date: str, out_dir: str | Path) -> list[Path]:
    ...
    for detail_url in detail_urls:
        detail_path = shimano_electric_detail.run(detail_url, crawl_date=crawl_date, out_dir=out_dir)
        product_raw = load_json(detail_path)

        spec_url = build_spec_url_from_detail_url(detail_url)
        spec_path = shimano_electric_spec.run(spec_url, out_dir=out_dir)
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Shimano 電動リール 一覧→詳細→spec pipeline")
