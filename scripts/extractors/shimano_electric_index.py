...
def fetch_html(url: str, timeout: int = 20) -> FetchResult:
    request = Request(
        ...
        headers={
            "User-Agent": (
                "Mozilla/5.0 (compatible; TsuriThreadsBot/0.1; +https://github.com/Cooker-fisher/fishing-threads.com)"
            )
        },
    )
...
def extract_product_nodes(soup: BeautifulSoup) -> list[Tag]:
    ...
    for anchor in soup.find_all("a", href=True):
        ...
        if not href or href in {"#", "/"}:
            continue
        if "next" in anchor_text.lower() or anchor_text.strip().isdigit():
            continue
...
def extract_item(node: Tag, base_url: str) -> dict[str, Any]:
    ...
    anchor = node.find("a", href=True, string=re.compile(r"VIEW PRODUCT", re.IGNORECASE))
    if anchor is None:
...
def save_index_raw(data: dict[str, Any], out_dir: str | Path) -> Path:
    ...
    out_path = target_dir / f"{page_no:03}.json"
...
def run(url: str, crawl_date: str, out_dir: str | Path) -> Path:
    ...
    items = [extract_item(node, url) for node in nodes]
    payload = build_index_raw(source_url=url, crawl_date=crawl_date, page_no=page_no, items=items)
    return save_index_raw(payload, out_dir)


def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Shimano 電動リール一覧 raw extractor")
    parser.add_argument("url", help="Shimano 電動リール一覧ページ URL")
